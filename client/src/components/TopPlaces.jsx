        ...
        <section className=' h-[300svh] w-screen overflow-x-hidden'>
            {filteredPlaces.map((place, index) => {
                ...
                return (
                    ...
                    <section className=' h-[100svh] overflow-hidden relative bg-black/40 flex items-center justify-center' key={index} >
                        <img src={`/sea${index}.jpg`} alt="Sea" className='absolute h-[100svh] w-screen object-cover -z-10' />
                        ...
                        <div className={` w-full px-20 flex ${index % 2 !== 0 ? 'flex-row' : 'flex-row-reverse'} max-lg:flex-col gap-8 items-center max-lg:justify-center backdrop-blur-sm h-full`}>
                            <Tilt className='flex-1 max-lg:flex-none overflow-hidden h-[22rem] w-[40rem] max-md:w-[30rem] max-md:h-[18rem] rounded-3xl'>
                                ...
                                <div className={`${alreadyLiked ? 'text-red-600' : 'text-white'} text-[2.7rem] absolute left-5 top-5 cursor-pointer hover:text-red-200 transition-all duration-300 ease-in-out`} onClick={() => like(place._id)}>
